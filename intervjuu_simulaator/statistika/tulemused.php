...
$stmt = $conn->prepare("SELECT s.id, s.kuupaev, s.oigesti, s.valesti, s.skoor, s.viimane_kysimus, s.oigesti_numbrid, s.valesti_numbrid, k.nimi, k.haridus, k.amet, k.huvid FROM skoorid s INNER JOIN kasutaja k ON s.kasutaja_id = k.id ORDER BY s.id desc");
$conn->set_charset("utf8");
echo $conn->error;
$stmt->execute();
$stmt->bind_result($id, $kuupaev, $oigesti, $valesti, $skoor, $viimane_kysimus, $oigesti_numbrid, $valesti_numbrid, $nimi, $haridus, $amet, $huvid);
