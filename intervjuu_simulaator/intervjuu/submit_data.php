...
$mysqli = new mysqli($server_host, $server_user_name, $server_password, $dbname);
...
if ($count == 0) {
  // Create a new row
  $sql = "INSERT INTO skoorid (skoor, kasutaja_id, oigesti, valesti, viimane_kysimus, oigesti_numbrid, valesti_numbrid) VALUES ('$score', '" . $_SESSION['kasutaja_id'] . "', '$correctAnswers', '$inCorrectAnswers', '$saveCurrentQuestionIndex', '$correctAnswersList', '$incorrectAnswersList')";
} else {
  // Update the existing row
  $sql = "UPDATE skoorid SET skoor = '$score', oigesti = '$correctAnswers', valesti = '$inCorrectAnswers', viimane_kysimus = '$saveCurrentQuestionIndex', oigesti_numbrid = '$correctAnswersList', valesti_numbrid = '$incorrectAnswersList' WHERE kasutaja_id = '" . $_SESSION['kasutaja_id'] . "'";
}
